...
const DigitalSection = require("../../models/Practice/digitalModel");
const azureBlobService = require("../../services/azureBlobService");

// Fetch the Technology Section
exports.getDigitalSection = async (req, res) => {
  try {
    let section = await DigitalSection.findOne();

    // If no section exists, create a default one
    if (!section) {
      section = await DigitalSection.create({
...
exports.upsertDigitalSection = async (req, res) => {
  try {
    ...
    const existingSection = await DigitalSection.findOne();
    ...
    const section = await DigitalSection.findOneAndUpdate({}, updateData, { new: true, upsert: true });
...
exports.addCard = async (req, res) => {
  try {
    ...
    const section = await DigitalSection.findOne();
    if (!section) {
